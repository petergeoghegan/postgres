			   ...
			   double *indexPages)
{
	 ...
	 * really aren't multiplicative in the way genericcostestimate expects.
	 * In general, most distinct combinations of SAOP keys will tend to not
	 * find any matching tuples.  Furthermore, btree scans search for the next
	 * set of array keys using the next tuple in line, and so won't even need
	 * a direct comparison to eliminate most non-matching sets of array keys.
	 *
