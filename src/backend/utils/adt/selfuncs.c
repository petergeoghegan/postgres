							 ...
							 Datum *endpointDatum)
{
	 ...
	 * a huge amount of time here, so we give up if we read a leaf page that
	 * yields no matching batch (all tuples killed or filtered).  When we fail
	 * for that reason, the caller will end up using whatever extremal value
	 * is recorded in pg_statistic.
	 */
	...
	/*
	 * Abort scan if the extremal index page has no matching tuples (excluding
	 * those already marked LP_DEAD).  This puts a ceiling on the total amount
	 * of work that we're willing to do.  If the extremal page has no usable
	 * value, we're likely dealing with a queue-like table with concentrated
	 * dead tuples at one end of the index.
	 */
