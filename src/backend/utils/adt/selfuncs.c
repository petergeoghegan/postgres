	...
	foreach(lc, path->indexclauses)
	{
		...
		if (indexcol != iclause->indexcol)
		{
			...
			while (indexcol < iclause->indexcol)
			{
				 ...
				 */
				thirdnpages = ceil(index->pages * 0.3333333);
				if (thirdnpages < new_num_sa_scans)
				{
					/*
					 * Cap the estimate to one third the number of pages so
					 * that costing is continuous, but saturates as the number
					 * of leaf pages that must be scanned approaches the
					 * maximum possible (i.e. approaches all leaf pages).
					 */
					num_sa_scans = Min(num_sa_scans, thirdnpages);
