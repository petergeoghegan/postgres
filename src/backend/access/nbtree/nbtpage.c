...
_bt_log_reuse_page(Relation rel, BlockNumber blkno, FullTransactionId safexid)
{
	...
	xlrec_reuse.latestCommittedXid = safexid;
 ...
 * generate their own latestCommittedXid by accessing the table directly,
 ...
 * the page, but it needs its own latestCommittedXid from caller (caller gets
 ...
 */
static void
_bt_delitems_delete(Relation rel, Buffer buf, TransactionId latestCommittedXid,
					OffsetNumber *deletable, int ndeletable,
					BTVacuumPosting *updatable, int nupdatable)
{
	Page		page = BufferGetPage(buf);
	...
	if (needswal)
	{
		...
		xlrec_delete.latestCommittedXid = latestCommittedXid;
						  ...
						  TM_IndexDeleteOp *delstate)
{
	...
	latestCommittedXid = table_index_delete_tuples(heapRel, delstate);

	/* Should not WAL-log latestCommittedXid unless it's required */
	if (!XLogStandbyInfoActive())
		latestCommittedXid = InvalidTransactionId;
	...
	_bt_delitems_delete(rel, buf, latestCommittedXid, deletable, ndeletable,
