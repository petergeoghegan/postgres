...
btfreebatch(IndexScanDesc scan, IndexScanBatch batch)
{
	...
	if (batch->opaque)
	{
		...
		/*
		 * XXX should we even have to do this here, within the index AM? Isn't
		 * this now rightfully the responsibility of indexam.c?
		 */
		Assert(!so->dropPin);	/* FIXME always false, for now */
		if (!so->dropPin)
			BTScanPosUnpin(*pos);
		...
		if ((batch = _bt_first(scan, ForwardScanDirection)))
		{
			...
			for (;;)
			{
				...
				if (++batch->itemIndex > batch->lastItem)
				{
					...
					batch = _bt_next(scan, batch->opaque,
		 ...
		 ScanKey orderbys, int norderbys)
{
	...
	/*
	 * TODO Move "dropPin" into IndexScanDesc, and make what used to be called
	 * _bt_drop_lock_and_maybe_pin into a generic routine, used by all AMs
	 * that support the new batch interface.  All of this stuff is 100% index
	 * AM agnostic already -- we just haven't generalized it before now.
	 */
	so->dropPin = false;
...
btrestrpos(IndexScanDesc scan)
{
	...
	batch = INDEX_SCAN_BATCH(scan, scan->xs_batches->markPos.batch);
	pos = (BTScanPos) batch->opaque;
