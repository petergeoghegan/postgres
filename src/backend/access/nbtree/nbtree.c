		 ...
		 IndexInfo *indexInfo)
{
	...
	if (!IsCatalogRelation(rel) && log_btree_verbosity >= 4)
	{
		debug = &debugstr;
...
btbeginscan(Relation rel, int nkeys, int norderbys)
{
	...
	so->log_array_advance = log_array_advance;
		 ...
		 ScanKey orderbys, int norderbys)
{
	...
	so->oppositeDirCheck = false;
	so->npages = 0;
	so->nforcenonrequiredpages = 0;
	so->nwrongprecheckpages = 0;
...
btendscan(IndexScanDesc scan)
{
	...
	if (so->log_btree_verbosity)
	{
		/* Don't \n here, since it's the last line of output: */
		appendStringInfo(&so->debugstr,
						 "btendscan for index \"%s\", ParallelWorkerNumber: %d, npages: %d, nforcenonrequiredpages: %d, nwrongprecheckpages: %d, nsearches: %lu\n",
						 RelationGetRelationName(scan->indexRelation),
						 ParallelWorkerNumber, so->npages, so->nforcenonrequiredpages,
						 so->nwrongprecheckpages,
						 scan->instrument ? scan->instrument->nsearches : -1);
