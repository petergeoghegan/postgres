			...
			Buffer buf)
{
	...
	if (P_ISLEAF(opaque))
	{
		...
		/*
		 * Return the best match we could find, which might point past the
		 * last slot (or before the first slot) from the leaf page
		 */
		return low;
	}
	 ...
	 * All truncated key attributes are interpreted as < the scan key -- even
	 * if the scan key omits entries for lower-order attributes.
	 *
	 * However, _bt_compare() will interpret truncated attributes as == any
	 * omitted scan key values during backward scans instead, which works just
	 * like >= or > here.  That isn't truly necessary, but it allows backward
	 * scans to avoid visiting an extra leaf page needlessly in certain cases.
	 */
