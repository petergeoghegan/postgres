	...
	if (ScanDirectionIsForward(dir))
	{
		...
		if (arrayKeys)
		{
			...
			if (!pstate.firstpage && so->skipScan && minoff < maxoff)
				_bt_forcenonrequired(scan, &pstate);
		}

		/*
		 * Only count a skipskip page when  _bt_scanbehind_checkkeys
		 * succeeeds
		 */
		if (pstate.forcenonrequired)
			so->nforcenonrequiredpages++;
		...
		if (pstate.continuescan && !so->scanBehind && !P_RIGHTMOST(opaque))
		{
			...
			{
				/*
				 * reset array keys so that _bt_checkkeys reliably restores
				 * the scan's required arrays to values >= finaltup
				 */
				Assert(so->skipScan);
				pstate.forcenonrequired = false;
				pstate.ikey = 0;
		...
		if (arrayKeys)
		{
			if (minoff <= maxoff && !P_LEFTMOST(opaque))
			{
					...
					!_bt_scanbehind_checkkeys(scan, dir, pstate.finaltup))
				{
					...
					{
						appendStringInfo(&so->debugstr,
										 "_bt_readpage: 🍀  %u with %u offsets/tuples (leftsib %u, rightsib %u) failed oppodir precheck and set so->currPos.moreLeft=false 🛑  ⬅️\n",
										 BufferGetBlockNumber(so->currPos.buf),
										 PageGetMaxOffsetNumber(page),
										 opaque->btpo_prev, opaque->btpo_next);
					}
...
_bt_steppage(IndexScanDesc scan, ScanDirection dir)
{
	...
	if (so->currPos.dir != dir)
	{
		...
		/*
		 * lastcurrblkno doesn't need to be removed from BMS to avoid false
		 * positives here; we might go back to it again (without currPos being
		 * invalidated first), but if we do we'll have to change directions
		 * once more (we'll del lastcurrblkno then)
		 */
		so->bmsPages = bms_del_member(so->bmsPages, (int) blkno);
	}
