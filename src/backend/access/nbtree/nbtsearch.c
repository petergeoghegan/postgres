...
_bt_search(Relation rel, Relation heaprel, BTScanInsert key, Buffer *bufP,
		   int access, Snapshot snapshot, StringInfo debugstr)
{
	...
	*bufP = _bt_getroot(rel, heaprel, access);
	...
	for (;;)
	{
		...
		*bufP = _bt_moveright(rel, heaprel, key, *bufP, (access == BT_WRITE),
							  stack_in, page_access, snapshot, debugstr);
	...
	if (ScanDirectionIsForward(dir))
	{
		...
		if (so->log_btree_verbosity)
		{
			...
			if (offnum <= maxoff)
			{
				...
				extra = psprintf(" from %s offnum %u TID (%u,%u) started page",
		...
		while (offnum <= maxoff)
		{
			...
			if (!pstate.continuescan)
			{
				...
				if (so->log_btree_verbosity)
				{
					...
					extra = psprintf(" from %s offnum %u TID (%u,%u) ended page and scan",
									 offnum < P_FIRSTDATAKEY(opaque) ? "high key" : "non-pivot",
									 offnum, itup_hblk, itup_hoff);
					print_blk_between_itups(&so->debugstr,
		...
		if (so->log_btree_verbosity && !continuescan_nonpivot)
		{
			if (!pstate.continuescan)
				print_blk_between_itups(&so->debugstr,
										...
										" _bt_readpage final: ",
										", continuescan high key check ended page and scan");
			else
				print_blk_between_itups(&so->debugstr,
										pstate.highkey, NULL, scan->indexRelation,
										" _bt_readpage final: ",
										", continuescan high key check did not end scan so must continue to right sibling in next _bt_readpage call, if any");
		}
	...
	else
	{
		...
		if (so->log_btree_verbosity)
		{
			...
			if (offnum >= minoff)
			{
				...
				itup_hblk = ItemPointerGetBlockNumber(htid);
				itup_hoff = ItemPointerGetOffsetNumber(htid);
				extra = psprintf(" from %s non-pivot offnum %u TID (%u,%u) started page",
								 offnum < P_FIRSTDATAKEY(opaque) ? "high key" : "non-pivot",
								 offnum, itup_hblk, itup_hoff);
				print_blk_between_itups(&so->debugstr,
										instr_itup, NULL, scan->indexRelation,
		 ...
		 * _bt_advance_array_keys_to_end when they reach the last page (the
		 * leftmost page) without any tuple ever setting continuescan to false
		 */
