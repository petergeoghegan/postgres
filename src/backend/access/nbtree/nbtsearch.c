 ...
 *		so->drop_pin scans, when we drop both the lock and the pin.
 ...
 * never locked. (Actually, when so->drop_pin there won't even be a pin held,
 ...
 * success exit (except during so->drop_pin=true index scans, when we drop the
 * pin eagerly to avoid blocking VACUUM).
 *
