...
_bt_first(IndexScanDesc scan, ScanDirection dir)
{
	...
	for (i = 0; i < keysz; i++)
	{
		...
		if (cur->sk_flags & SK_ROW_HEADER)
		{
			...
			if (so->log_btree_verbosity)
			{
				...
				appendStringInfo(&so->debugstr,
								 "%ssk_attno %d. val: %s, func: %s\n",
								 i == 0 ? "_bt_first: " : "           ",
								 subkey->sk_attno, val, fname);
			...
			if (i == keysz - 1)
			{
				...
				for (;;)
				{
					...
					if (so->log_btree_verbosity)
					{
						...
						val = OidOutputFunctionCall(typOutput, subkey->sk_argument);
						appendStringInfo(&so->debugstr,
										 "%ssk_attno %d. extra val: %s, func: %s\n",
										 i == 0 ? "_bt_first: " : "           ",
										 subkey->sk_attno, val, fname);
						if (val)
							pfree(val);
	...
	if (so->log_btree_verbosity)
	{
		...
		strat = dump_scankey_strategy(&ss);
