...
dump_scankey_flags(ScanKey cur)
{
	...
	if (cur->sk_flags & SK_BT_NEG_INF)
		appendStringInfo(&output, "SK_BT_NEG_INF, ");
	if (cur->sk_flags & SK_BT_POS_INF)
		appendStringInfo(&output, "SK_BT_POS_INF, ");
	if (cur->sk_flags & SK_BT_NEXTKEY)
		appendStringInfo(&output, "SK_BT_NEXTKEY, ");
	if (cur->sk_flags & SK_BT_PREVKEY)
		appendStringInfo(&output, "SK_BT_PREVKEY, ");
...
_bt_first(IndexScanDesc scan, ScanDirection dir)
{
	...
	for (i = 0; i < keysz; i++)
	{
		...
		if (cur->sk_flags & SK_ROW_HEADER)
		{
			...
			if (so->log_btree_verbosity)
			{
				char	   *fname = get_func_name(inskey.scankeys[i].sk_func.fn_oid);
				char	   *flags = dump_scankey_flags(&inskey.scankeys[i]);
				...
				appendStringInfo(&so->debugstr,
								 ...
								 i == 0 ? "_bt_first: " : "           ",
								 subkey->sk_attno, val, fname, flags);
			...
			if (i == keysz - 1)
			{
				...
				for (;;)
				{
					...
					if (so->log_btree_verbosity)
					{
						char	   *fname = get_func_name(inskey.scankeys[keysz - 1].sk_func.fn_oid);
						char	   *flags = dump_scankey_flags(&inskey.scankeys[keysz - 1]);
						...
						val = OidOutputFunctionCall(typOutput, subkey->sk_argument);
						appendStringInfo(&so->debugstr,
										 "%ssk_attno %d. extra val: %s, func: %s, flags: %s\n",
										 i == 0 ? "_bt_first: " : "           ",
										 subkey->sk_attno, val, fname, flags);
						if (val)
							pfree(val);
