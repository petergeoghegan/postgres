 ...
 * right sibling link still points to lastcurrblkno, the page that we just
 * finished reading.  Otherwise, we have to figure out which page is the
 * correct one to visit next the hard way, which requires reasoning about both
 * concurrent page splits and concurrent page deletions.  See nbtree/README.
 *
