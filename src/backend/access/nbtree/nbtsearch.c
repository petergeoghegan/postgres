...
_bt_first(IndexScanDesc scan, ScanDirection dir)
{
	...
	firstbatch = indexam_util_batch_alloc(MaxTIDsPerBTreePage,
										  scan->xs_want_itup);
				  ...
				  OffsetNumber offnum, ScanDirection dir)
{
	...
	if (_bt_readpage(scan, firstbatch, dir, offnum, true))
	{
		...
		indexam_util_batch_unlock(rel, scan->batchState && scan->batchState->dropPin,
								  firstbatch);
		return firstbatch;
	}
				 ...
				 BlockNumber lastcurrblkno, ScanDirection dir, bool firstpage)
{
	...
	newbatch = indexam_util_batch_alloc(MaxTIDsPerBTreePage,
										scan->xs_want_itup);
