...
_bt_readpage(IndexScanDesc scan, ScanDirection dir, OffsetNumber offnum)
{
	...
	if (ScanDirectionIsForward(dir))
	{
		...
		{
			appendStringInfo(&so->debugstr,
							 "_bt_readpage: leaf page %u with %u offsets/tuples (leftsib %u, rightsib %u)\n",
			...
			{
				appendStringInfo(&so->debugstr,
								 "       first: none, offnum %u is > maxoff of %u\n",
								 offnum, maxoff);
			}
		}
		...
		if (so->log_btree_verbosity && !continuescan_nonpivot)
		{
			if (!continuescan)
				print_blk_between_itups(&so->debugstr,
										highkey, NULL, scan->indexRelation,
										"       final: ",
	...
	else
	{
		...
		if (so->log_btree_verbosity)
		{
			 ...
			 */
			appendStringInfo(&so->debugstr,
							 "_bt_readpage: leaf page %u with %u offsets/tuples (rightsib %u, leftsib %u)\n",
							 BufferGetBlockNumber(so->currPos.buf),
							 PageGetMaxOffsetNumber(page),
			...
			{
				appendStringInfo(&so->debugstr,
								 "       first: none, offnum %u is < minoff of %u\n",
								 offnum, minoff);
			}
		}
	...
	{
		appendStringInfo(&so->debugstr,
						 "       stats: currPos.firstItem: %d, currPos.lastItem: %d, nmatching: %d",
