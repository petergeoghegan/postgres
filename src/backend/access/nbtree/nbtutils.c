					   ...
					   IndexTuple tuple, int sktrig)
{
	...
	if (so->log_btree_verbosity >= 2)
	{
		...
		appendStringInfo(&so->debugstr,
						 "  _bt_advance_array_keys: all_eqtype_sk_equal is %d\n",
						 all_eqtype_sk_equal);
	}
	...
	if (all_eqtype_sk_equal)
	{
		...
		if (likely(_bt_check_compare(dir, so, tuple, ntupatts, itupdesc,
									 &continuescan, &insktrig, false, scan)))
			return true;
		...
		!_bt_tuple_before_array_skeys(scan, pstate, pstate->finaltup, 0))
	{
		...
		_bt_check_compare(flipped, so, pstate->finaltup, nfinaltupatts,
						  itupdesc, &continuescan, &opsktrig, false, scan);
	...
	for (key = so->keyData, ikey = 0; ikey < so->numberOfKeys; key++, ikey++)
	{
		...
		*sktrig = ikey;
		...
		if (!DatumGetBool(test))
		{
			...
			if (so->log_btree_verbosity >= 2)
				appendStringInfo(&so->debugstr,
								 "   ikey %d/sk_attno %d: (sk_argument %lu) result: false, with continuescan=%d\n",
