		...
		if (cur->sk_flags & SK_SEARCHARRAY)
		{
			if (so->log_btree_verbosity >= 2)
			{
				...
				appendStringInfo(&so->debugstr,
								 "_bt_preprocess_array_keys: SK_SEARCHARRAY input %d: [ strategy: %s, attno: %u, func: %s flags: %s ]\n",
								 i, strat, cur->sk_attno, fname, flags);
				pfree(flags);
				pfree(strat);
				if (fname)
					pfree(fname);
	...
	if (scan->numberOfKeys > 0 && !IsSystemRelation(rel))
	{
		...
		for (int i = 0; i < scan->numberOfKeys; i++)
		{
			...
			while (last_equal_temp < cur->sk_attno)
			{
				...
				if (opcintype != INT4OID && opcintype != DATEOID)
				{
							   ...
							   bool *qual_ok)
{
	...
	if (array->num_elems == -1)
	{
		...
		/* XXX use proper comparator here */
		*qual_ok = ((int32) array->min_value <= (int32) array->max_value);
		return true;
	}
					   ...
					   int32 *set_elem_result, IndexScanDesc scan)
{
	...
	if (array->num_elems == -1)
	{
		...
		if (tupnull)
			elog(WARNING, "tried to binary search for NULL tuple");
...
_bt_preprocess_keys(IndexScanDesc scan)
{
	...
	if (so->log_btree_verbosity)
	{
		for (int inkeysn = 0; inkeysn < numberOfKeys; inkeysn++)
		{
			ScanKey		j = scan->keyData + inkeysn;
			 ...
			 */
			appendStringInfo(&so->debugstr,
							 "_bt_preprocess_keys:  scan->keyData[%d]: [ strategy: %s, attno: %u func: %s, flags: %s ]\n",
							 inkeysn, strat, j->sk_attno, fname, flags);
	...
	if (so->log_btree_verbosity)
	{
			 ...
			 instrcur++, ikey++)
		{
			...
			appendStringInfo(&so->debugstr,
							 "_bt_preprocess_keys:    so->keyData[%d]: [ strategy: %s, attno: %u func: %s, flags: %s ]\n",
