 ...
 * for the current array keys.  finaltup is either the page high key (during
 * forwards scans) or the first non-pivot tuple (during backwards scans).
 *
 * Called during scans with array keys, when the so->scanBehind flag was set
