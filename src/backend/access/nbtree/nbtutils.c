		...
		if (cur->sk_flags & SK_SEARCHARRAY)
		{
			if (so->log_btree_verbosity >= 2)
			{
				...
				appendStringInfo(&so->debugstr,
								 "_bt_preprocess_array_keys: SK_SEARCHARRAY input %d: [ strategy: %s, flags: %s, attno: %u func OID: %u ]\n",
								 i, strat, flags, cur->sk_attno,
								 cur->sk_func.fn_oid);
				pfree(flags);
				pfree(strat);
			}
					   ...
					   int sktrig)
{
		...
		(!sktrig_required && all_satisfied))
	{
		...
		if (_bt_check_compare(dir, so, tuple, tupnatts, tupdesc,
							  false, false, false,
							  &pstate->continuescan, &nsktrig, scan) &&
			!so->scanBehind)
		{
		...
		(all_required_satisfied || oppodir_inequality_sktrig))
	{
		...
		_bt_check_compare(flipped, so, pstate->finaltup, nfinaltupatts,
						  ...
						  &continuescanflip, &opsktrig, scan);
...
_bt_preprocess_keys(IndexScanDesc scan)
{
	...
	if (so->log_btree_verbosity >= 2)
	{
		...
		for (int inkeysn = 0; inkeysn < scan->numberOfKeys; inkeysn++)
		{
			 ...
			 */
			appendStringInfo(&so->debugstr,
							 "_bt_preprocess_keys:  inkeys[%d]: [ strategy: %s, flags: %s, attno: %u func OID: %u ]\n",
							 inkeysn, strat, flags, instrcur->sk_attno,
	...
	if (so->log_btree_verbosity >= 2)
	{
			 ...
			 instrcur++, ikey++)
		{
			...
			appendStringInfo(&so->debugstr,
							 "_bt_preprocess_keys: outkeys[%d]: [ strategy: %s, flags: %s, attno: %u func OID: %u ]\n",
							 ikey, strat, flags, instrcur->sk_attno,
							 instrcur->sk_func.fn_oid);
			pfree(flags);
			pfree(strat);
		}
			  ...
			  IndexTuple tuple, int tupnatts)
{
	...
	if (pstate->prechecked || pstate->firstmatch)
	{
		bool		dcontinuescan;
		int			dikey = 0;
		int			log_btree_verbosity = so->log_btree_verbosity;	/* save */

		so->log_btree_verbosity = 0;	/* silence */
		...
		so->log_btree_verbosity = log_btree_verbosity;	/* restore */
	}
#endif
