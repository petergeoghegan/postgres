 ...
 * following a set of btgettuple calls made after _bt_readpage returned true
 * and before _bt_next needed to step off the so->currPos page.
 *
...
_bt_killitems(IndexScanDesc scan)
{
	 ...
	 * items must appear in page offset number order.  Posting list tuple
	 * entries from the same tuple/page offset must appear in heap TID order.
	 */
	...
	for (int i = so->currPos.firstItem; i <= so->currPos.lastItem; i++)
	{
		...
		while (offnum <= maxoff)
		{
			...
			if (BTreeTupleIsPosting(ituple))
			{
				...
				{
					ItemPointer itemptr = BTreeTupleGetPostingN(ituple, j);

					if (!kitem->itemDead || kitem->indexOffset != offnum ||
						!ItemPointerEquals(itemptr, &kitem->heapTid))
