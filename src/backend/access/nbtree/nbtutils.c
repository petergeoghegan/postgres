...
_bt_preprocess_array_keys(IndexScanDesc scan, int *numberOfKeys)
{
	...
	for (int input_ikey = 0; input_ikey < scan->numberOfKeys; input_ikey++)
	{
			   ...
			   attno_skip <= scan->keyData[input_ikey].sk_attno)
		{
			...
			so->arrayKeys[numArrayKeys].sksup.low_compare = NULL;	/* for now */
			so->arrayKeys[numArrayKeys].sksup.high_compare = NULL;	/* for now */
		...
		switch (skey->sk_strategy)
		{
			...
			case BTLessEqualStrategyNumber:
				array->sksup.high_compare =
					MemoryContextAlloc(so->arrayContext, sizeof(ScanKeyData));
				memcpy(array->sksup.high_compare, skey, sizeof(ScanKeyData));
				break;
			case BTEqualStrategyNumber:
			...
			case BTGreaterStrategyNumber:
				array->sksup.low_compare =
					MemoryContextAlloc(so->arrayContext, sizeof(ScanKeyData));
				memcpy(array->sksup.low_compare, skey, sizeof(ScanKeyData));
				break;
			default:
				elog(ERROR, "unrecognized StrategyNumber: %d",
						   ...
						   int32 *set_elem_result)
{
	...
	if (!array->use_sksup)
	{
		...
		{
			bool		low_satisfied = true,
						high_satisfied = true;

			if (array->sksup.low_compare)
			{
				ScanKey		low_compare = array->sksup.low_compare;

				low_satisfied =
					DatumGetBool(FunctionCall2Coll(&low_compare->sk_func,
												   low_compare->sk_collation,
												   tupdatum,
												   low_compare->sk_argument));
			}
			if (array->sksup.high_compare)
			{
				ScanKey		high_compare = array->sksup.high_compare;

				high_satisfied =
					DatumGetBool(FunctionCall2Coll(&high_compare->sk_func,
			...
			if (ScanDirectionIsForward(dir))
			{
				if (!high_satisfied)
				{
					/* Caller needs to perform "beyond end" advancement */
					*set_elem_result = 1;
					return;
				}
				if (!low_satisfied)
									 ...
									 FmgrInfo *orderproc)
{
	...
	{
		/*
		 * Scan key/array lacks a skip support routine.  Have to use sentinel
		 * values instead of using real incremented datum values (no way to
		 * generate real datums without help from the opclass).
		 */
		if ((ScanDirectionIsForward(dir) && (skey->sk_flags & SK_BT_POS_INF)) ||
			(ScanDirectionIsBackward(dir) && (skey->sk_flags & SK_BT_NEG_INF)))
			goto rollover;
		...
		if (skey->sk_flags & SK_ISNULL)
		{
				 ...
				 !(skey->sk_flags & SK_BT_NULLS_FIRST)))
			{
				skey->sk_flags &= ~(SK_SEARCHNULL | SK_ISNULL |
									SK_BT_NEG_INF | SK_BT_POS_INF |
									SK_BT_NEXTKEY | SK_BT_PREVKEY);
				skey->sk_flags |= SK_SEARCHNOTNULL | SK_ISNULL;
				if (ScanDirectionIsForward(dir))
					skey->sk_flags |= SK_BT_NEXTKEY;
				else
					skey->sk_flags |= SK_BT_PREVKEY;
		...
		if (ScanDirectionIsForward(dir) && array->sksup.high_compare)
		{
			ScanKey		high_compare = array->sksup.high_compare;
		...
		if (ScanDirectionIsBackward(dir) && array->sksup.low_compare)
		{
			ScanKey		low_compare = array->sksup.low_compare;
		...
		/* "Increment/decrement" using a next key/prev key sentinel */
		if (ScanDirectionIsForward(dir))
			skey->sk_flags |= SK_BT_NEXTKEY;
		else
			skey->sk_flags |= SK_BT_PREVKEY;
		...
		for (int i = 0; i < so->numArrayKeys; i++)
		{
			...
			if (skey->sk_flags & SK_ISNULL)
			{
				appendStringInfo(&so->debugstr,
								 ...
								 (skey->sk_flags & SK_BT_NEXTKEY) != 0 ? " SK_BT_NEXTKEY" : "",
								 (skey->sk_flags & SK_BT_PREVKEY) != 0 ? " SK_BT_PREVKEY" : "",
								 (skey->sk_flags & SK_BT_NEG_INF) != 0 ? " SK_BT_NEG_INF" : "",
								 (skey->sk_flags & SK_BT_POS_INF) != 0 ? " SK_BT_POS_INF" : "",
								 arrk->scan_key == sktrig ? "            <--" : "");
				continue;
			}

			if (skey->sk_argument == 0)
				val = NULL;
			else
			{
...
_bt_preprocess_keys(IndexScanDesc scan)
{
	...
	if (so->log_btree_verbosity)
	{
		for (int ikey = 0; ikey < so->numberOfKeys; ikey++)
		{
			ScanKey		instrcur = &so->keyData[ikey];
			TupleDesc	tupdesc = RelationGetDescr(scan->indexRelation);
