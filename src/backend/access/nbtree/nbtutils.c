...
new_prim_scan:
	...
	/*
	 * Consider backing out of performing another primitive index scan when it
	 * looks like we should favor continuing the ongoing primscan instead.
	 *
	 * Back out like this (change our mind about starting a primitive scan)
	 * when we see that we already made the same mistake, if it also appears
	 * likely that we're about to repeat the same mistake again now.  Also do
	 * so when we've committed to sticking to the leaf level for now.
	 */
	if (so->lastNextPage == so->currPos.currPage &&
		(tuple == pstate->finaltup || pstate->force))
	{
		so->noSkipskipNext = !pstate->force;

		/* schedule a recheck once on the next (or previous) page */
		so->scanBehind = true;
		so->forceNext = true;

		/* We've change our mind: we'll continue the current primitive scan */
		goto continue_scan;
	}
			  ...
			  IndexTuple tuple, int tupnatts)
{
	...
	if (!arrayKeys && so->numArrayKeys)
	{
		 ...
		 */
		Assert(!so->scanBehind && !so->forceNext);
