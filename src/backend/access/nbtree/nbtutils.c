 ...
 * also be larger when we added a skip array/skip scan key.
 *
...
_bt_preprocess_array_keys(IndexScanDesc scan, int *numberOfKeys)
{
	...
	/* Tell caller the final number of arrayKeyData[] keys on output */
	*numberOfKeys = output_ikey;

	/*
	 * Might need to enlarge the preallocated so->keyData[] space that our
	 * caller outputs final scan keys into.  btbeginscan optimistically sizes
	 * the buffer to fit every scan->keyData[] entry, which doesn't account
	 * for the possible addition of scan keys used by skip arrays.
	 */
	...
	for (int i = 0;; inputsk++, i++)
	{
		...
		while (attno_skip < attno_inputsk)
		{
			if (!_bt_skip_support(rel, attno_skip, &skipatts[attno_skip - 1]))
			{
				 ...
				 * skip arrays that were provisionally added by any previous
				 * iterations of this innermost loop (during the current/final
				 * iteration of outermost loop).  Adding those skip arrays can
				 * only be useful when they form a contiguous group that
				 * includes all missing attributes (all attributes up to and
				 * including the one immediately before attno_inputsk).
				 */
