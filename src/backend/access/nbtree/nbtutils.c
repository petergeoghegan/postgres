...
_bt_preprocess_keys(IndexScanDesc scan)
{
	...
	if (so->log_btree_verbosity)
	{
		for (int ikey = 0; ikey < numberOfKeys; ikey++)
		{
			...
			if (jskey->sk_flags & SK_ROW_HEADER)
			{
				ScanKey		subkey = (ScanKey) DatumGetPointer(jskey->sk_argument);

				for (;;)
				{
					flags = dump_scankey_flags(subkey);
					strat = dump_scankey_strategy(subkey);
					fname = get_func_name(subkey->sk_func.fn_oid);
					attr = TupleDescAttr(tupdesc, subkey->sk_attno - 1);

					Assert(subkey->sk_flags & SK_ROW_MEMBER);

					appendStringInfo(&so->debugstr,
									 "                               sub key: [ strategy: %s, attno: %u/\"%s\", func: %s, flags: %s ]\n",
									 strat, subkey->sk_attno,
									 NameStr(attr->attname), fname, flags);
	...
	if (so->log_btree_verbosity)
	{
		for (int ikey = 0; ikey < so->numberOfKeys; ikey++)
		{
			...
			if (instrcur->sk_flags & SK_ROW_HEADER)
			{
				ScanKey		subkey = (ScanKey) DatumGetPointer(instrcur->sk_argument);
