...
_bt_preprocess_array_keys(IndexScanDesc scan, int *numberOfKeys)
{
	...
	if (scan->numberOfKeys > 0 && !IsSystemRelation(rel) && !scan->parallel_scan)
	{
		...
		for (int i = 0;; cur++, i++)
		{
			 ...
			 * cross-type scenarios (at least with range style skip arrays).
			 * Paper over it on !USE_FLOAT8_BYVAL platforms, which have test
			 * failures as a result of the sloppy approach taken within
			 ...
			 * _bt_skip_scankey_preprocess is taught to set min_value and
			 * max_value in a way that accounts for cross-type differences
			 * within a B-Tree opfamily.
			 */
 ...
 * into each array's min_value and max_value fields to deal with range type
