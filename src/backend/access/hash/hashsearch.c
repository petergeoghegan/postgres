...
_hash_next(IndexScanDesc scan, ScanDirection dir, BatchIndexScan priorbatch)
{
	...
	/*
	 * Determine which page to read next based on scan direction and
	 * prior batch navigation information.
	 */
	...
	/* Allocate batch for the next page */
	batch = indexam_util_batch_alloc(scan, MaxIndexTuplesPerPage, false);

	/* Read the next page and load items into batch */
 ...
 *
 *		On failure exit (no more tuples), we return NULL, with pin held on
			   ...
			   BatchIndexScan batch)
{
	...
	if (ScanDirectionIsForward(dir))
	{
		...
		for (;;)
		{
			...
			itemIndex = _hash_load_qualified_items(scan, page, offnum, dir, batch);
