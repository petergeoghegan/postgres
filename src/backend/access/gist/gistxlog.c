...
gistRedoPageReuse(XLogReaderState *record)
{
	 ...
	 *
	 * latestCommittedXid was the page's deleteXid.  The
	 ...
	 */
	if (InHotStandby)
		ResolveRecoveryConflictWithSnapshotFullXid(xlrec->latestCommittedXid,
												   xlrec->locator);
}
 ...
 */
void
gistXLogPageReuse(Relation rel, BlockNumber blkno,
				  FullTransactionId latestCommittedXid)
{
	...
	xlrec_reuse.latestCommittedXid = latestCommittedXid;
			   ...
			   TransactionId latestCommittedXid)
{
	 ...
	 * buffer, to allow us to find the latestCommittedXid on a standby server.
	 */
