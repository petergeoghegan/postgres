 ...
 * for the full specification of this contract.
 *
 ...
 *
 * Note: amunguardbatch must be idempotent on the index AM side -- it may be
 * called more than once for the same batch, and must do nothing if the
 * interlock has already been dropped.
 */
void
tableam_util_unguard_batch(IndexScanDesc scan, IndexScanBatch batch)
{
	/* Only supposed to be called during !batchImmediateUnguard scans */
 ...
 * resource recycling.  Index AMs implementing amgetbatch should generally use
 * these rather than managing buffers directly.
 * ----------------------------------------------------------------
 */
