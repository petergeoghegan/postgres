...
batch_getnext(IndexScanDesc scan, ScanDirection direction)
{
	...
	if (batch != NULL)
	{
		...
		if (priorbatch && !scan->xs_heapfetch->rs && !batchqueue->disabled &&
			!scan->xs_want_itup && enable_indexscan_prefetch)
...
index_batch_restore_pos(IndexScanDesc scan)
{
	 ...
	 *
	 * FIXME Temporarily disabled due to breaking streamBatch used by
	 * prefetching
	 */
 ...
 * rather than managing buffers directly.  The dropPin flag in BatchQueue
 * controls pin management strategy to optimize for different scan types.
 * ----------------------------------------------------------------
 */
 ...
 * will either be a newly allocated batch, or a batch reused from a cache of
 * batches already freed by calling indexam_util_batch_release.  See comments
 * above indexam_util_batch_release.
 *
 * We assume that all calls here during the same index scan will always use
 * the same maxitems and want_itup arguments.  Index AMs that uses batches
