...
indexam_util_alloc_batch(IndexScanDesc scan)
{
	...
	if (!batch)
	{
		 ...
		 * fixed-size opaque area into a single offset that can be used to
		 * find the true allocation base from the batch pointer.  (This is
		 * also where the fixed-size table AM opaque area can be found.)
		 */
		if (scan->batch_base_offset == 0)
		{
			uint16		table_opaque = 0;
			Size		per_item_area = 0;

			/*
			 * The table AM's per-item area immediately follows its fixed-size
			 * header, so the table AM can reach it at a fixed offset without
			 * storing a pointer.
			 */
			if (scan->usebatchring)
			{
				table_opaque = scan->batch_opaque_size;
				per_item_area = MAXALIGN((Size) scan->batch_per_item_size *
										 scan->maxitemsbatch);
			}

			/* batch_base_offset is uint16: */
			Assert((Size) table_opaque + per_item_area +
				   scan->batch_index_opaque_size <= PG_UINT16_MAX);

			scan->batch_base_offset = table_opaque + per_item_area +
	 ...
	 * don't have to provide any opaque data.  They can even choose whether
	 * each scan uses a fixed and/or per-item opaque area dynamically, within
	 * their table_index_fetch_begin callback (which was called already).
	 *
	 * We always avoid calling table_index_fetch_batch_init during bitmap
	 * scans.  There won't have been any call to table_index_fetch_begin.
	 * (Index AMs generally reuse the same batch infrastructure internally for
	 * bitmap index scans, but that's just for convenience.)
	 */
	if (scan->usebatchring &&
