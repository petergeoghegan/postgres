 ...
 *		slot from.  "name" columns stored as cstrings (e.g. btree name_ops)
 *		are re-padded to NAMEDATALEN allocations, which live in the scan's
 *		per-tuple xs_name_cstring_cxt (reset here on each call).
 *
 *		This is only intended to be called by table AMs, in their
 *		xs_getnext_slot callbacks; the executor receives a filled slot from
 *		table_index_getnext_slot instead.
 * ----------------
 */
void
index_fill_ios_slot(IndexScanDesc scan, TupleTableSlot *slot)
{
	/* index-only scans use a virtual slot (see table_index_getnext_slot) */
	...
	{
		/*
		 * We generally expect the slot tupdesc's data types to be datatype
		 * compatible with the tupdesc supplied by the index AM (if that
		 * cannot be guaranteed then the AM should use xs_hitup/xs_hitupdesc
		 * instead), which is something we can't so easily check.  They better
		 * have the same number of columns, too, which we can assert here.
		 *
		 * Note: there is one special case that we'll handle below: opclasses
		 * such as btree/name_ops, which index "name" columns using "cstring"
		 * storage.  Our handling preserves the general guarantee that datums
		 * will always be in the representation the slot's types expect.
		 */
		Assert(slot->tts_tupleDescriptor->natts == scan->xs_itupdesc->natts);

		/*
		 * We must use the tupdesc supplied by the AM in index_deform_tuple,
