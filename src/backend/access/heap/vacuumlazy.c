...
lazy_scan_heap(LVRelState *vacrel)
{
	...
	next_block_to_scan = lazy_scan_skip(vacrel, 0, &all_visible);
	for (blkno = 0; blkno < rel_pages; blkno++)
	{
		...
		/*
		 * Must scan this page.  But determine the next block in line to be
		 * scanned according to vmsnap/lazy_scan_skip first.
		 */
		all_visible_according_to_vmsnap = all_visible;
		next_block_to_scan = lazy_scan_skip(vacrel, blkno + 1, &all_visible);
