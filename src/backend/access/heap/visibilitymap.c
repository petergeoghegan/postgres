 ...
 * visibility map bit gets concurrently unset.
 *
 * LOCKING
 *
	...
	*allvisible = block.all_visible;
	...
	if (vmsnap->next_prefetch_idx < vmsnap->first_invalid_idx)
	{
		...
		elog(DEBUG_ELOG, "scan prefetch block: %u (just about to return next block %u to vacuum)",
			 prefetch.scanned_block, next_block_to_scan);
