...
heapam_index_fetch_next_batch(IndexScanDesc scan, IndexFetchHeapData *hscan,
							  ScanDirection direction,
							  IndexScanBatch priorBatch,
							  BatchRingItemPos *pos)
{
 ...
 * off scanBatch to an earlier index page, heapam_index_fetch_next_batch will
								   ...
								   bool *all_visible)
{
	 ...
	 */
	scanBatch = heapam_index_fetch_next_batch(scan, hscan, direction,
											  scanBatch, scanPos);
								 ...
								 void *per_buffer_data)
{
	 ...
	 * the same heap block", so this is safe).
	 *
	...
	for (;;)
	{
										 ...
										 prefetchBatch, prefetchPos))
		{
			...
			prefetchBatch = heapam_index_fetch_next_batch(scan, hscan,
														  xs_read_stream_dir,
														  prefetchBatch,
														  prefetchPos);
			if (!prefetchBatch)
