...
static bool
heap_batch_advance_pos(IndexScanDesc scan, struct BatchQueueItemPos *streamPos,
					   ScanDirection direction)
{
	...
	if (INDEX_SCAN_POS_INVALID(streamPos))
	{
		 ...
		 */
		streamBatch = INDEX_SCAN_BATCH(scan, scan->batchqueue->headBatch);

		streamPos->batch = scan->batchqueue->headBatch;
