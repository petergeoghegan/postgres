 ...
 *
 * Note: visibilitymap_get_status does not lock the visibility map buffer, so
 * the result could be slightly stale.  See the "Memory ordering effects"
 * discussion in visibilitymap_get_status's header comment for why this is
 * safe for index-only scans.
 */
static void
 ...
 * index scan.  Called each time a new batch is obtained from the index AM.
 *
 ...
 *
 * We also avoid prefetching during scans where we're unable to drop each
 * batch's buffer pin right away (non-MVCC snapshot scans).  We are not
								  ...
								  IndexScanBatch priorBatch)
{
	...
	/* else don't start a read stream just yet */
}

/*
 * Get the next batch of TIDs from a scan.
 *
...
heapam_index_batch_getnext(IndexScanDesc scan, ScanDirection direction,
						   IndexScanBatch priorBatch, BatchRingItemPos *pos)
{
	...
	if (batch)
	{
		...
		heapam_index_consider_prefetching(scan, hscan, priorBatch);
	}
	else
	{
 ...
 * off scanBatch to an earlier index page, heapam_index_batch_getnext will
								   ...
								   bool *all_visible)
{
	 ...
	 */
	scanBatch = heapam_index_batch_getnext(scan, direction, scanBatch, scanPos);
								 ...
								 void *per_buffer_data)
{
		...
		index_scan_pos_cmp(prefetchPos, scanPos, xs_read_stream_dir) < 0)
	{
		...
		*prefetchPos = *scanPos;
		...
		if (scan->xs_want_itup)
		{
			...
			/* Make heapam_index_batch_pos_visibility release resources eagerly */
			hscan->xs_vm_items = scan->maxitemsbatch;
	...
	for (;;)
	{
		 ...
		 * scans).  The read stream tries to be careful about not pinning too
		 * many buffers, and that's harder to do reliably if there are
		 * variable numbers of pins taken without such care.
		 */
										 ...
										 prefetchBatch, prefetchPos))
		{
			...
			prefetchBatch = heapam_index_batch_getnext(scan, xs_read_stream_dir,
													   prefetchBatch,
													   prefetchPos);
			if (!prefetchBatch)
			{
				/*
				 * No more batches in this direction, so all the batches
				 * that the scan will ever require (barring a change in
				 * scan direction) are now loaded
				 */
...
/*
 * Common implementation for all four index-scan heap-fetch variants.
 *
						  ...
						  bool amgetbatch)
{
	...
	for (;;)
	{
		...
		if (index_only)
		{
			 ...
			 * on which all tuples are known visible to everybody.  In any
			 * case, we'll use the index tuple not the heap tuple as the
			 * data source.
			 */
			if (!all_visible)
			{
				...
				/*
				 * Only MVCC snapshots are supported with standard
				 * index-only scans, so there should be no need to keep
				 * following the HOT chain once a visible entry has been
				 * found.  Other callers (currently only selfuncs.c) use
				 * SnapshotNonVacuumable, and want us to assume that just
				 * having one visible tuple in the hot chain is always good
				 * enough.
				 */
			...
			/*
			 * Return matching index tuple now set in scan->xs_itup (or
			 * return matching heap tuple now set in scan->xs_hitup).
			 */
			return true;
		}
		else
...
heapam_index_plain_ambatch_getnext_slot(IndexScanDesc scan,
										ScanDirection direction,
										TupleTableSlot *slot)
{
...
static pg_attribute_hot bool
heapam_index_only_ambatch_getnext_slot(IndexScanDesc scan,
									   ScanDirection direction,
									   TupleTableSlot *slot)
{
	Assert(scan->xs_want_itup && scan->usebatchring);
