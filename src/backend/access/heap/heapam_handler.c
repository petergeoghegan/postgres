...
heapam_index_fetch_begin(Relation rel)
{
	...
	hscan->xs_base.ioss_VMBuffer = InvalidBuffer;
...
heapam_index_fetch_end(IndexFetchTableData *scan)
{
	...
	if (scan->ioss_VMBuffer != InvalidBuffer)
	{
		ReleaseBuffer(scan->ioss_VMBuffer);
		scan->ioss_VMBuffer = InvalidBuffer;
	}
...
index_getnext_slot(IndexScanDesc scan, ScanDirection direction,
				   TupleTableSlot *slot)
{
	IndexFetchTableData *xs_heapfetch = scan->xs_heapfetch;
	...
	for (;;)
	{
		if (!scan->xs_heap_continue)
		{
			 ...
			 *
			 * Scans that use a legacy amgettuple index AM simply call through
			 * to index_getnext_tid to get the next TID returned by index AM.
			 * The progress of the scan will be under the control of index AM
			 * (we just pass it through a direction to get the next tuple in),
			 * so we cannot reorder any work.
			 */
			if (scan->batchqueue != NULL)
		...
		else
		{
			 ...
			 */
			if (!VM_ALL_VISIBLE(xs_heapfetch->rel,
								ItemPointerGetBlockNumber(tid),
								&xs_heapfetch->ioss_VMBuffer))
			{
				/*
				 * Rats, we have to visit the heap to check visibility.
				 */
				xs_heapfetch->nheapaccesses++;
				if (!index_fetch_heap(scan, xs_heapfetch->ioss_TableSlot))
					continue;		/* no visible tuple, try next index entry */
				if (!IsMVCCSnapshot(scan->xs_snapshot))
					return true;

				ExecClearTuple(xs_heapfetch->ioss_TableSlot);
			...
			else
			{
				 ...
				 * that at page level.
				 */
				PredicateLockPage(xs_heapfetch->rel,
								  ItemPointerGetBlockNumber(tid),
								  scan->xs_snapshot);
			}
			return true;
