						  ...
						  Buffer *vmbuffer, Buffer *vmbuffer_other)
{
	 ...
	 * doesn't work, we ask the Free Space Map to locate a suitable page. If
	 * the FSM has no record of a page with enough free space, we give up and
	 * extend the relation.
	 */
	...
	if (targetBlock == InvalidBlockNumber)
	{
		 ...
		 * target.
		 */
		targetBlock = GetPageWithFreeSpace(relation, targetFreeSpace, bistate);

		/* Bulk extend the FSM if that didn't work */
		if (targetBlock == InvalidBlockNumber)
			goto extend;
	}
...
newtarget:
	...
	/*
	 * Read and exclusive-lock the target block, as well as the other block if
	 * one was given, taking suitable care with lock ordering and the
	 ...
	 * time we return, we'll have x-locked the buffer, and we don't want to do
	 * any I/O while in that state.  So we check the bit here before taking
	 * the lock, and pin the page if it appears necessary.  Checking without
	 * the lock creates a risk of getting the wrong answer, so we'll have to
	...
	/*
	 * We now have the target page (and the other buffer, if any) pinned and
	 * locked.  However, since our initial PageIsAllVisible checks were
	 * performed before acquiring the lock, the results might now be out of
	 * date, either for the selected victim buffer, or for the other buffer
	 * passed by the caller.  In that case, we'll need to give up our locks,
	 * go get the pin(s) we failed to get earlier, and re-lock.  That's pretty
	 * painful, but hopefully shouldn't happen often.
	 *
	 * Note that there's a small possibility that we didn't pin the page above
	 * but still have the correct page pinned anyway, either because we've
	 * already made a previous pass through this loop, or because caller
	 * passed us the right page anyway.
	 *
	 * Note also that it's possible that by the time we get the pin and retake
	 * the buffer locks, the visibility map bit will have been cleared by some
	 * other backend anyway.  In that case, we'll have done a bit of extra
	 * work for no gain, but there's no real harm done.
	 */
	if (otherBuffer == InvalidBuffer || targetBlock <= otherBlock)
		GetVisibilityMapPins(relation, buffer, otherBuffer, targetBlock,
							 otherBlock, vmbuffer, vmbuffer_other);
	else
		GetVisibilityMapPins(relation, otherBuffer, buffer, otherBlock,
							 targetBlock, vmbuffer_other, vmbuffer);

	/*
	 * Now we can check to see if there's enough free space here. If so, we're
	 * done.
	 */
	 ...
	 * dirtying the buffer here, and rely on the caller to do so whenever it
	 * puts a tuple onto the page, but there seems not much benefit in doing
	 * so.
	 */
	if (PageIsNew(page))
	{
	 ...
	 * and prepare to look elsewhere.  We don't care which order we unlock the
	 * two buffers in, so this can be slightly simpler than the code above.
	 */
...
extend:
	 ...
	 *
	 * XXX This needs !ConditionalLockRelationForExtensionOrWait() function,
	 * akin to LWLockAcquireOrWait().
	 */
	...
	/*
	 * If some other waiter has already extended the relation now that we have
	 * the lock, we don't need to do so; just use the existing freespace.  If
	 * we find that somebody else has put free space in the FSM for us when we
	 * acquire the extension lock, we start from scratch.
	 */
	targetBlock = GetPageWithFreeSpace(relation, targetFreeSpace, bistate);
	if (targetBlock != InvalidBlockNumber)
	{
	 ...
	 */
	buffer = FreeSpaceMapAddExtraBlocks(relation, targetFreeSpace, bistate);
	page = BufferGetPage(buffer);
