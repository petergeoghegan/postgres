						  ...
						  Buffer *vmbuffer, Buffer *vmbuffer_other)
{
	...
	else if (otherBuffer == InvalidBuffer)
	{
		...
		targetFreeSpace = minFreeSpace + len * 2;
	}
	else
	{
		minFreeSpace = len;	/* UPDATEs don't need extra saveFreeSpace */
		targetFreeSpace = Max(minFreeSpace, saveFreeSpace * 0.75);
	}
		...
		if (minFreeSpace <= pageFreeSpace)
		{
			...
			else
			{
				...
				if (targetFreeSpace > pageFreeSpace)
					newspace = (int) pageFreeSpace - (len + sizeof(ItemIdData));
				else
					newspace = (int) pageFreeSpace - (int) targetFreeSpace;
