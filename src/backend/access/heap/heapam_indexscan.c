						  ...
						  bool amgetbatch)
{
	...
	for (;;)
	{
		 ...
		 * only do it with a non-MVCC snapshot).  During an amgetbatch
		 * index-only scan, the TID's all_visible status comes along with it.
		 */
		...
		/*
		 * Load hscan only now, after the index AM call, so that the compiler
		 * needn't preserve it across that call
		 */
		hscan = (IndexScanHeapData *) scan->xs_table_opaque;

		/*
		 * The scan's next TID was set in scan->xs_heaptid for us.  An
		 * amgettuple index-only scan checks its heap page's visibility here.
		 */
		...
		else
		{
			...
			if (unlikely(!all_visible))
			{
				if (!heapam_index_only_heap_fetch(scan, amgetbatch))
				{
					 ...
					 * (only selfuncs.c's index-only scans do this), give up
					 * once the scan has switched heap pages too many times.
					 *
					 * This is the only use of hscan after the heap fetch
					 * call. Reload it, rather than reusing the value loaded
					 * before the call, so that the compiler needn't keep it
					 * in a callee-saved register across the call.  That would
					 * cost every returned tuple a register save and restore,
					 * not just the rare ones that get here.
					 */
					hscan = (IndexScanHeapData *) scan->xs_table_opaque;
			 ...
			 *
			 * index_fill_ios_slot is the authority on how to do this; we just
			 * open-code its common IndexTuple case (no heap-format tuple, no
			 * "name" columns needing re-padding) to avoid paying for a
			 * per-tuple call
			 */
			ExecClearTuple(slot);
...
/*
 * Out-of-line heapam_index_heap_fetch for index-only scans.  Also takes care
 * of the instrumentation, and of killing the index entry when the whole HOT
 * chain turned out to be dead, so that the caller keeps nothing around for it.
 */
...
heapam_index_only_heap_fetch(IndexScanDesc scan, bool amgetbatch)
{
	...
	if (heapam_index_heap_fetch(scan, hscan, NULL, true,
								&scan->xs_heap_continue, &all_dead))
		return true;
