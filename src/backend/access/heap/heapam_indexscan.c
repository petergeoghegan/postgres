...
static inline HeapBatchData *heapam_index_batch_data(IndexScanDesc scan,
...
heapam_index_fetch_batch_init(IndexScanDesc scan, IndexScanBatch batch,
							  bool new_alloc)
{
	HeapBatchData *hbatch = heapam_index_batch_data(scan, batch);
...
heapam_index_batch_data(IndexScanDesc scan, IndexScanBatch batch)
{
								...
								bool *all_visible)
{
	 ...
	 */
	hbatch = heapam_index_batch_data(scan, scanBatch);
								  ...
								  BatchRingItemPos *pos)
{
	...
	Assert(hbatch == heapam_index_batch_data(scan, batch));
		...
		index_scan_pos_cmp(prefetchPos, scanPos, xs_read_stream_dir) < 0)
	{
		...
		*prefetchPos = *scanPos;
		...
		if (scan->xs_want_itup)
		{
			...
			{
				HeapBatchData *hbatch = heapam_index_batch_data(scan,
																prefetchBatch);
