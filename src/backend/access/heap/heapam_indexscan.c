								   ...
								   ScanDirection direction, bool *all_visible)
{
	 ...
	 */
	if (tableam_util_scanpos_advance(scan, direction, &scanBatch))
	{
		/*
	...
	/*
	 * Determine if tableam_util_scanpos_advance just failed because there was
	 * no existing loaded scanBatch for it to consider
	 */
	hadExistingScanBatch = (scanBatch != NULL);
	 ...
	 * yet.
	 *
	 * If it wasn't the scan's first returned batch, consider if we should
	 * begin prefetching now (unless prefetching is already underway).
	 */
	heapam_index_consider_prefetching(scan, hscan, hadExistingScanBatch);

	/*
	 * Update batchringbuf and scanPos such that the scan can continue with
	 * our new scanBatch.
	 *
...
heapam_index_dirchange_reset(IndexScanDesc scan, IndexFetchHeapData *hscan,
							 ScanDirection direction)
{
								 ...
								 void *per_buffer_data)
{
	...
	ScanDirection dir = hscan->xs_read_stream_dir;
	IndexScanBatch prefetchBatch;
	HeapBatchData *hbatch = NULL;

	/* The scan direction must already be established */
	 ...
	 * scanPos when it isn't already (validly) ahead of scanPos.  This is what
	 * happens on the first call here for the scan; see
	 * tableam_util_prefetchpos_catchup for the full details.
	 *
	 * Initialization is a special case: we return prefetchPos's item's heap
	 * block directly, rather than prefetching ahead.  In other words, we
	 * return the heap block from the TID passed to heapam_index_heap_fetch at
	 * the point where it called read_stream_next_buffer (typically the first
	 * such call for the entire scan, made shortly after the scan's read
	 * stream is created).
	 */
	if (tableam_util_prefetchpos_catchup(scan, dir))
	{
		...
		if (scan->xs_want_itup)
		{
			...
			if (prefetchBatch->isGuarded)
				heapam_index_batch_pos_visibility(scan, dir,
		 ...
		 */
		Assert(index_scan_pos_cmp(scanPos, prefetchPos, dir) == 0);
	...
	for (;;)
	{
		...
		switch (tableam_util_prefetchpos_advance(scan, dir, &prefetchBatch))
		{
