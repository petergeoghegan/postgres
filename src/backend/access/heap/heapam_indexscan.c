								 ...
								 void *per_buffer_data)
{
	...
	for (;;)
	{
		 ...
		 * call here already advanced prefetchBatch twice.  This is possible
		 * during index-only scans with long runs of batches containing only
		 * items that are all-visible (it's also possible during plain index
		 * scans with unusual batch layouts, though that's much less common).
		 *
