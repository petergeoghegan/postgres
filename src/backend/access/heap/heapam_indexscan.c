...
heapam_index_fetch_batch_init(IndexScanDesc scan, IndexScanBatch batch,
							  bool new_alloc)
{
	HeapBatchData *batchvis = index_scan_batch_table_area(scan, batch);
	...
	if (scan->xs_want_itup)
	{
		if (new_alloc)
		{
			...
			batchvis->visInfo = (uint8 *) ((char *) batch + itemsEnd);
		}

		/* Clear visibility flags (needed for both new and recycled batches) */
		memset(batchvis->visInfo, 0, scan->maxitemsbatch);
	}
	else
	{
		batchvis->visInfo = NULL;
	}
}
								...
								bool *all_visible)
{
	...
	if (!(batchvis->visInfo[scanPos->item] & HEAP_BATCH_VIS_CHECKED))
		heapam_index_batch_pos_visibility(scan, direction, scanBatch, batchvis,
										  scanPos);

	/* Finally, set all_visible for caller */
	*all_visible =
		(batchvis->visInfo[scanPos->item] & HEAP_BATCH_VIS_ALL_VISIBLE) != 0;
...
heapam_index_batch_pos_visibility(IndexScanDesc scan, ScanDirection direction,
								  IndexScanBatch batch, HeapBatchData *batchvis,
								  BatchRingItemPos *pos)
{
	IndexFetchHeapData *hscan = (IndexFetchHeapData *) scan->xs_heapfetch;
	...
	if (ScanDirectionIsForward(direction))
	{
		...
		for (int setItem = posItem; setItem <= lastSetItem; setItem++)
		{
			...
			/* Must never overwrite any batch item's visInfo */
			if (batchvis->visInfo[setItem] & HEAP_BATCH_VIS_CHECKED)
				continue;

			if (heapblkno == curvmheapblkno)
			{
				batchvis->visInfo[setItem] = curvmheapblkflags;
				continue;
			}
			...
			batchvis->visInfo[setItem] = curvmheapblkflags = flags;
			curvmheapblkno = heapblkno;
		}

		allbatchitemsvisible = lastSetItem >= batch->lastItem &&
			(posItem == batch->firstItem ||
			 (batchvis->visInfo[batch->firstItem] & HEAP_BATCH_VIS_CHECKED));
	}
	else
	{
		...
		allbatchitemsvisible = lastSetItem <= batch->firstItem &&
			(posItem == batch->lastItem ||
			 (batchvis->visInfo[batch->lastItem] & HEAP_BATCH_VIS_CHECKED));
	}
	...
	if (allbatchitemsvisible)
	{
		...
		for (int i = batch->firstItem; i <= batch->lastItem; i++)
			Assert(batchvis->visInfo[i] & HEAP_BATCH_VIS_CHECKED);
#endif
								 ...
								 void *per_buffer_data)
{
		...
		index_scan_pos_cmp(prefetchPos, scanPos, xs_read_stream_dir) < 0)
	{
		...
		*prefetchPos = *scanPos;
		...
		if (scan->xs_want_itup)
		{
			 ...
			 * overwriting the existing visInfo[] for scanPos/prefetchPos.
			 */
			hscan->xs_vm_items = scan->maxitemsbatch;
		...
		/*
		 * The item can't be all-visible according to visInfo[] -- if it were,
		 * we'd never have been called in the first place
		 */
		Assert(!batchvis ||
			   !(batchvis->visInfo[prefetchPos->item] & HEAP_BATCH_VIS_ALL_VISIBLE));
	...
	for (;;)
	{
		...
		/* First, try incrementing prefetchPos */
		...
		{
			Assert(scan->xs_want_itup);
			Assert(batchvis->visInfo[prefetchPos->item] & HEAP_BATCH_VIS_CHECKED);

			if (batchvis->visInfo[prefetchPos->item] & HEAP_BATCH_VIS_ALL_VISIBLE)
			{
