...
typedef struct
{
	...
	TransactionId latestCommittedXid;	/* latest committed xid pruned away */
				...
				OffsetNumber *off_loc)
{
	...
	prstate.latestCommittedXid = InvalidTransactionId;
	...
	if (prstate.nredirected > 0 || prstate.ndead > 0 || prstate.nunused > 0)
	{
		...
		if (RelationNeedsWAL(relation))
		{
			...
			xlrec.latestCommittedXid = prstate.latestCommittedXid;
	...
	if (ItemIdIsNormal(rootlp))
	{
		...
		if (HeapTupleHeaderIsHeapOnly(htup))
		{
				...
				!HeapTupleHeaderIsHotUpdated(htup))
			{
				...
				HeapTupleHeaderAdvanceLatestCommittedXid(htup,
														 &prstate->latestCommittedXid);
				ndeleted++;
			}
