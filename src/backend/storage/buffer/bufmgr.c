			...
			IOContext io_context)
{
	 ...
	 * "fake" LSNs generated by GetFakeLSNForUnloggedRel.  It is unlikely but
	 * possible that the fake LSN counter could advance past the WAL insertion
	 * point; and if it did happen, attempting to flush WAL through that
	 * location would fail, with disastrous system-wide consequences.  To make
	 * sure that can't happen, skip the flush if the buffer isn't permanent.
	 */
