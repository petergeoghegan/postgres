...
static bool ios_prefetch_block(IndexScanDesc scan, void *arg, BatchQueueItemPos *pos);
...
IndexOnlyNext(IndexOnlyScanState *node)
{
	...
	if (scandesc == NULL)
	{
		 ...
		 */
		if (scandesc->batchqueue != NULL)
		{
			scandesc->batchqueue->prefetch = ios_prefetch_block;
			scandesc->batchqueue->prefetchArg = (void *) node;
		}
...
#define INDEX_SCAN_BATCH(scan, idx)	\
		((scan)->batchqueue->batches[(idx) % (scan)->batchqueue->maxBatches])
 ...
 */
static bool
ios_prefetch_block(IndexScanDesc scan, void *arg, BatchQueueItemPos *pos)
{
	...
	if (batch->itemsvisibility[pos->item] == IOS_UNKNOWN_VISIBILITY)
	{
		bool		all_visible;
		ItemPointer tid = &batch->items[pos->item].heapTid;

		all_visible = VM_ALL_VISIBLE(scan->heapRelation,
									 ItemPointerGetBlockNumber(tid),
									 &node->ioss_VMBuffer);

		batch->itemsvisibility[pos->item] =
			all_visible ? IOS_ALL_VISIBLE : IOS_NOT_ALL_VISIBLE;
	}

	/* prefetch only blocks that are not all-visible */
	return (batch->itemsvisibility[pos->item] == IOS_NOT_ALL_VISIBLE);
}
