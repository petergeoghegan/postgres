			   ...
			   int elevel)
{
	 ...
	 * usually does no work.  (This includes less frequent maintenance tasks
	 * such as page deletion/recycling, which B-Tree indexes can do when
	 * amvacuumcleanup() is called by VACUUM or VACUUM ANALYZE.)
	 */
	if (!(params->options & VACOPT_VACUUM))
	{
		for (ind = 0; ind < nindexes; ind++)
