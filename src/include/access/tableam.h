...
typedef struct TableAmRoutine
{
	 ...
	 *
	 * Called by batchscan_alloc for each new or recycled batch, but only when
	 * the table AM reserved an opaque area for the scan (by setting
 ...
 * Initialize table AM's per-batch opaque area within a batch allocation.
 *
 * Called by batchscan_alloc for each new or recycled batch, but only
