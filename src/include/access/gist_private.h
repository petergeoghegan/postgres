 ...
 *
 * Ordered (nearest-neighbor) scans use "virtual batches", which work
 * completely differently.  They drain the pairing-heap queue and pack
 * matching leaf heap items into these batch in exact distance order (rather
 * than one physical leaf page per batch).  This is safe because of various
 * restrictions: index-only scans (which table AMs need to be able to unguard
 * themselves) aren't supported at all, and calls to gistkillitemsbatch are
 * handled as no-ops when a virtual batch is passed.
 */
...
/*
 * Access GiST's per-item data for a batch item.  The index AM's dynamic opaque
 * area is our own array of GISTBatchItem, variable-width due to distances[], so
 * we subscript it ourselves.
 */
