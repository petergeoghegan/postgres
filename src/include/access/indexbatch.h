...
/*
 * index_batch_get_amdata - access the index AM's per-batch opaque data.
 *
 ...
 * This macro returns a typed pointer to that area.  It also asserts that the
 * scan's batch_index_opaque_size matches the expected type size, catching
 * mismatches between the index AM's ambeginscan setup and the accessor.
 */
#define index_batch_get_amdata(scan, batch, type) \
