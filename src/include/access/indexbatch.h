							  ...
							  IndexScanBatch priorBatch, BatchRingItemPos *pos)
{
	...
	if (batch)
	{
		...
		/*
		 * We don't set knownEndForward/knownEndBackward to false (whichever
		 * is used when moving in the opposite direction) here when this is
		 * the scan's first returned batch.  The index AM will generally do so
		 * for us (though we don't rely on that for correctness).
		 */
	}
	else
	{
