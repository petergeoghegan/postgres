 ...
 * position operations (index_scan_*), the protocol functions used by table
 * AMs to manage an index scan's positional state (tableam_util_*, the larger
 * of which appear in indexbatch.c), and utilities called by index AMs that
 * implement amgetbatch (indexam_util_*).
 *
 * The data structures that these functions operate on (IndexScanBatchData,
 * BatchRingBuffer, and related structs) are defined in access/relscan.h, not
 * here: IndexScanDescData embeds them by value, so they must be visible
 * wherever scan descriptors are.  relscan.h defines the data structures;
 * this module provides all of the code that operates on them.
 *
 ...
 * is accessed via index_scan_batch_index_opaque_static at that fixed offset.
 ...
 * area, which sits immediately before the batch pointer.
 *
 * We assert in passing that 'sizeof(type)' matches batch_index_opaque_static.
 * Index AMs use their own wrapper function-style macro, built on top of this.
 */
 ...
 * the ring buffer.  We'll have also set pos's item offset to newBatch's first
 * item in the given direction (which is actually newBatch's _last_ item when
 * scanning backwards).
 *
 ...
 * its ring buffer slot.
 *
 * Returns true when a previously occupied ring buffer slot was freed.  A
 * table AM that paused its prefetch mechanism because the ring buffer was
 * full (see tableam_util_prefetchpos_advance) can resume it now that there's
 * space to store one more batch.  Returns false when newScanBatch is the
 * scan's first batch (there was no old scanBatch for us to release).
 */
static pg_attribute_always_inline bool
...
{
	BatchRingItemPos *prefetchPos = &scan->batchringbuf.prefetchPos;
	...
	if (!index_scan_pos_advance(direction, *prefetchBatch, prefetchPos))
	{
		...
		if (unlikely(index_scan_batch_full(scan)))
		{
			 ...
			 * batch slots are currently in use.  Undo the most recent advance
			 * before returning, leaving prefetchPos in a state that's
			 * consistent with the work actually performed (various positional
			 * state assertions expect it).
			 */
			if (ScanDirectionIsForward(direction))
			{
		...
		/* else we have a free ring buffer slot to fit another batch */
