 ...
 * details of the preprocessing), information about the current location of
 * the scan, and information about the marked location, if any.
 *
 * Index scans work a page at a time: we pin and read-lock the page, identify
 * all the matching items on the page and save them, then release the
 * read-lock while returning the items to the caller for processing.  This
 * approach minimizes lock/unlock traffic.  We must always drop the lock to
 * make it okay for caller to process the returned items.  Whether or not we
 * can also release the pin during this window will vary.  We drop the pin
 * (when dropPin is set in batch state) to avoid blocking progress by VACUUM
...
extern BatchIndexScan btgetbatch(IndexScanDesc scan, BatchIndexScan batch,
								 ScanDirection dir);
