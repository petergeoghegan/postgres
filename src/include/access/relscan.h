 ...
 *   [table AM per-item area]  <- per-item data (e.g., for visibility info)
 ...
 * plus its per-item area) and the index AM opaque area into a single offset
 * from the batch pointer to the true allocation base.  The indexbatch.c
 * utilities pfree a batch by passing pfree a pointer returned by
 ...
 * shim accessor.  Its fixed-size header (which is sized batch_opaque_size,
 * possibly zero) is immediately followed by its per-item area (which is sized
 * maxitemsbatch * batch_per_item_size, and can also be zero).  Because the
 * per-item area is immediately after the fixed-size header, the table AM can
 * reach it at an offset from the fixed header.  In general, the table AM's
 * table_index_fetch_begin callback has complete freedom to vary the layout of
 * its opaque area based on the details of the scan.
 *
 * The index AM opaque area is accessed via a custom accessor that uses a
 * fixed compile-time constant offset (tracked in the scan descriptor as
 * batch_index_opaque_size).  It always sits immediately before the batch
 * pointer, regardless of how the table AM chooses its opaque area layout.
 * This scheme is more efficient but less flexible than the one for table AMs:
 * it forces index AMs to always use the same generic fixed-size header.
 */
