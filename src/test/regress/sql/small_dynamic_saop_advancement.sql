...
----------------------------------------------------
-- Weird IS NOT NULL bug in skipskip optimization --
----------------------------------------------------
set client_min_messages=error;
drop table if exists onek_with_null_skipskip;
reset client_min_messages;

create unlogged table onek_with_null_skipskip as select unique1, unique2 from onek_skipscan ;
insert into onek_with_null_skipskip (unique1,unique2) values (null, -1), (null, null);

create unique index first_skipskip_idx on onek_with_null_skipskip (unique2,unique1);

-- with skip support:
set skipscan_skipsupport_enabled=true;
select count(*) from onek_with_null_skipskip where unique1 is not null and unique1 > 500;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
select count(*) from onek_with_null_skipskip where unique1 is not null and unique1 > 500;

-- without skip support:
set skipscan_skipsupport_enabled=false;
select count(*) from onek_with_null_skipskip where unique1 is not null and unique1 > 500;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
select count(*) from onek_with_null_skipskip where unique1 is not null and unique1 > 500;

-- set up for second index:
reset skipscan_skipsupport_enabled;
drop index first_skipskip_idx;
create unique index second_skipskip_idx  on onek_with_null_skipskip (unique2  nulls first,unique1);
