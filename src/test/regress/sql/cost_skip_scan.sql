...
-- # Final 10, where we totally omit "ten" column (approximately the same cost
-- as prior range skip array "#10" query is expected, which will be a little
-- bit more costly due to having to find the min and max "ten" vals)
select *
from cost_of_skipping
where
  hundred = 4 and thousand = 35 and ten_thousand = 36 and i = 441;
EXPLAIN (ANALYZE, BUFFERS, SUMMARY OFF)
select *
from cost_of_skipping
where
...
-- # Final 10, where "ten" column has non-excluding lower bound only
select *
from cost_of_skipping
where
ten > -100 and
...
-- # Final 10, where "ten" column has non-excluding upper bound only
select *
from cost_of_skipping
where
ten < 100 and
...
-------------------------------------------------------------
-- Redundant operators should not affect costing test case --
-------------------------------------------------------------

-- Baseline, all later queries are semantically equivalent in spite of
-- redundancies, and so should have exactly the same cost as this baseline:
-- (XXX Actually, not quite, since the optimizer won't credit nbtree with its
-- ability to eliminate redundant operators later on -- but still very close,
-- selectivity estimates are exact matches at least)
select *
from cost_of_skipping
where
ten >= 2 and ten <= 7 and
...
-- Different operators, same meanings:
select *
from cost_of_skipping
where
ten > 1 and ten < 8 and
...
-----------------------------------------------------------------
-- END Redundant operators should not affect costing test case --
-----------------------------------------------------------------
