...
-- XXX here unique1 ends up using a true inequality in so->keyData[], which
-- means that we won't detect contradictoriness on the basis of this being out
-- of range for int4.
select unique1
from tenk1_skipscan
...
-- prove contradictoriness on the basis of int4 range:
--
-- (XXX Update July 22) Actually, this is no longer proven contradictory,
-- following changes to range skip arrays and inequalities that made skip
-- support arrays work more like non-skip-support arrays.  Live with it for
-- now.
select unique1
from tenk1_skipscan
where unique1 > 2147483647
...
-- Contradictory qual:
-- FIXME here unique1 ends up using a true inequality in so->keyData[], which
-- means that we won't detect contradictoriness on the basis of this being
-- contradictory in a way that involves >= and <= operators (not just same
-- strategy/operator).
...
-- Missing predicate is in "intermediate" column (ten) here:
-- XXX "#define FORCE_NOSKIP_DEBUG + integer wisconsin table" broke with this
-- query, but this seems to be fine, for whatever reason:
