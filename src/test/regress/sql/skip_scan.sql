...
limit 3;
-- FIXME we used to detect contradictory quals here, but since we stopped
-- using skip arrays (preferring inequality scan keys) for last attribute
-- covered by any scan key that is no longer the case.
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
select unique1
from tenk1_skipscan
where unique1 > 2147483647
...
where unique1 >= 2147483647
limit 3;
-- NOFIXME This didn't change when we started using inequality scan keys again
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
select unique1
from tenk1_skipscan
where unique1 >= 2147483647
