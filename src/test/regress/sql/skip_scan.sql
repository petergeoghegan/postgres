...
-- (September 11 2025)
-- Test case showing assertion failure due to incorrect handling of DESC
-- columns when preprocessing increments/changes operator of scan key
-- incorrectly
--
-- Per https://postgr.es/m/19049-b7df801e71de41b2@postgresql.org
--
-- Set up a table with >=2 DESC index keys
set client_min_messages=error;
drop table if exists desc_preproc_bug;
reset client_min_messages;
create unlogged table desc_preproc_bug (t1 int, t2 int, s0 int, s1 int);
create index desc_preproc_bug_idx on desc_preproc_bug(s0, s1, t1 desc, t2 desc);
insert into desc_preproc_bug select f.t,  f.t, 1 s0, 1 s1 from generate_series(1, 2 * pow(10, 4)::int) f(t);
vacuum freeze desc_preproc_bug;

-- crashed with an Assert failure:
select distinct on (s0, s1) *
from desc_preproc_bug
where t2 >= -1 and t1 < 10
order by s0, s1, t1 desc, t2 desc;
