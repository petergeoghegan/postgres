...
-- (June 18)
--
-- Verify that we're not adding a bunch of useless skip scan arrays -- if
-- they're not making at least one = operator required then they must be
-- useless
select
  *
from
...
where
  b = '1995-01-01' -- can skip "a"
  and
  (d, e) > (3, 44); -- can't skip row compare, so shouldn't be skipping "c", either
select
  *
from
  scan_key_lowest_highest_opclass_func_test
where
  (d, e) > (3, 44); -- can't skip row compare, so shouldn't be "a", "b", or "c", either
...
from
  scan_key_lowest_highest_opclass_func_test
where
  noskiptwo = 'foo'; -- can't skip "noskipone" so shouldn't get any skip arrays
