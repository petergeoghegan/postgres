...
execute tenth;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF) -- Nice win: 91 hits for patch vs 710 for master
execute tenth;
deallocate tenth;
...
execute eleventh;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF) -- nice win, 91 for patch vs 710 for master
execute eleventh;
deallocate eleventh;
...
-------------------------------------------------------------------------------------
-- iprefix_sales_mdam_paper can't-reset-arrays with iprefix optimization test case --
-------------------------------------------------------------------------------------

-- (March 14, 2025)
--
-- This test case demonstrates how it was possible for the
-- _bt_skip_ikeyprefix/pstate.forcenonrequired optimization/commit to alter
-- the primitive index scan scheduling of certain queries.
--
-- Recall that this "Lower the overhead of skip array maintenance" commit is
-- supposed to not have any affect whatsoever on primscan scheduling; it's
-- just supposed to keep the overhead low (it might actually be a bit
-- counter-productive, at times, since it means that we can't use the
-- also-valuable lookahead optimization, but that's not in scope for this
-- test case -- it's accepted as a cost of doing business at this point).
set client_min_messages=error;
drop table if exists iprefix_sales_mdam_paper;
reset client_min_messages;

-- Load table
create unlogged table iprefix_sales_mdam_paper
(
  dept int4,
  sdate date,
  omitted_truncated_column int4,
  equals_truncated_column int4
);
create index iprefix_idx on iprefix_sales_mdam_paper(dept, sdate, omitted_truncated_column, equals_truncated_column);

insert into iprefix_sales_mdam_paper (dept, sdate, omitted_truncated_column, equals_truncated_column)
select
  dept,
  '1995-01-01'::date + sdate,
  omitted_truncated_column,
  equals_truncated_column
from
  generate_series(1, 1) dept,
  generate_series(1, 292) sdate,
  generate_series(1, 5) omitted_truncated_column,
  generate_series(1, 291) equals_truncated_column;
vacuum (analyze, freeze) iprefix_sales_mdam_paper;

-- This is :rootitems, just to avoid test regressions:
select itemoffset, ctid, itemlen, nulls from bt_page_items('iprefix_idx', (select fastroot::int4 from bt_metap('iprefix_idx')));
...
select
  ...
  equals_truncated_column
from
  iprefix_sales_mdam_paper
where
  dept = 1
  and sdate = '1995-01-02'
  and equals_truncated_column = 50;

execute iprefix_confusion;

-- With the optimization enabled we get 4 index searches and 14 buffer hits,
-- just like when it is forcibly disabled via set "skipscan_iprefix_enabled=false":
--
-- XXX UPDATE (May 6 2025): recall that the details here changed when you went
-- back to calling _bt_start_array_keys within _bt_readpage, to recover from
-- the use fo pstate.forcenonrequired mode.  We got back to consistent
-- behavior here (i.e. consistent number of hits/primscans independent of the
-- skipscan_iprefix_enabled setting) by checking truncated
-- attributes/scanBehind from within _bt_scanbehind_checkkeys.
set skipscan_iprefix_enabled=true; -- redundant
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
execute iprefix_confusion;

-- This is the baseline case, which always worked correctly, and that we
-- expect the prior case to match exactly in terms of # of primscans, etc:
set skipscan_iprefix_enabled=false;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
execute iprefix_confusion;
