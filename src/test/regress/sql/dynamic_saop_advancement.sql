...
set enable_saop_optimization=on;
...
-- Versus master/no optimization, which only has 4:
set enable_saop_optimization=off;
select * from skippy_tbl where bar = any ('{366,2147483647}');
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
select * from skippy_tbl where bar = any ('{366,2147483647}');  -- no speculation
set enable_saop_optimization=on; -- reset
...
-- Simpler case
-- As above, just with fewer 'b' constants (otherwise we get extra "Filter:"
-- conditions).
...
--
-- Here we used to not generate an index path with clauses for
-- (two,four,twenty), all because "four" doesn't appear in query (only "two"
-- and "twenty" appear).  My heuristic that helped elsewhere (when the "gap"
-- column was an inequality instead of an equality constraint) must not do the
-- wrong thing here -- the most selective path is one with all 3 clauses.
--
-- This bug was fixed by restoring the notion of two passes that each generate
-- distinct SAOP index paths, while holding onto the idea that the first pass
-- could sometimes use more than the first index column as an index qual.
...
--
-- Allowing the optimizer to include "twenty" in its own index qual forces it
-- to also recognize that the higher order clause for the column "four" is
-- safe to include in a sortable index path -- even though "four" is omitted
...
--
-- Patch doesn't do all that much better than master here (124 buffer hits vs
...
-- Even an inequality on two will work, since we can get through non-matches
...
-- We can also mix non-SOAPs with SOAPs -- example #1:
select ctid, thousand from tenk1_dyn_saop
where
  two = 0 and four = 2 and twenty in (1, 2)
order by two, four, twenty limit 20;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
select ctid, thousand from tenk1_dyn_saop
where
...
-- We can also mix non-SOAPs with SOAPs -- example #2:
select ctid, thousand from tenk1_dyn_saop
where
  two = 0 and four in (1, 2) and twenty = 2
order by two, four, twenty limit 20;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
select ctid, thousand from tenk1_dyn_saop
where
...
-- Should be able to support this kind of ROW() comparison:
...
-- RowCompare variant
-- Make sure that we get "Filter: (anotherorderid = ANY ('{1,2}'::integer[]))"
-- here, just like master branch, since dynamic SAOP execution isn't safe.
-- Trying to do make it happen in the planner cannot possibly help, and just
-- increases work for nbtree (since it knows not to try it itself).
