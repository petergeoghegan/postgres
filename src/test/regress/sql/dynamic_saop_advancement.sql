...
-- Wait until every dead tuple has become removable by VACUUM, so that VACUUM
-- can set all-visible bits.  Autovacuum can hold a snapshot open in any
-- database at any time, which holds back the removable cutoff and makes
-- "Heap Fetches: 0" (and index-only scan buffer counts) flaky.  Call this
-- immediately before any VACUUM whose visibility map state the test relies on.
create or replace procedure wait_prunable() language plpgsql as $$
  declare
    barrier xid;
  begin
    barrier := pg_current_xact_id()::xid;
    loop
      rollback;  -- release MyProc->xmin, which could be the oldest
      exit when not exists (
        select 1 from pg_stat_activity
        where pid <> pg_backend_pid()
          and (age(backend_xmin) > age(barrier) or age(backend_xid) > age(barrier)));
      raise log 'wait_prunable: waiting for snapshots older than %', barrier;
...
select pg_stat_get_xact_idx_blocks_hit('must_not_full_scan'::regclass) as cur_blocks_hit \gset
...
and orderid in (48, 50)
order by district, warehouse, orderid, orderline;
select pg_stat_get_xact_idx_blocks_hit('backup_wrong_idx'::regclass) as cur_blocks_hit \gset
...
select pg_stat_get_xact_idx_blocks_hit('plain_equal_inequal_nonrequired'::regclass) as cur_blocks_hit \gset
\set old_blocks_hit :cur_blocks_hit
...
select pg_stat_get_xact_idx_blocks_hit('buggy_idx'::regclass) as cur_blocks_hit \gset
\set old_blocks_hit :cur_blocks_hit
...
2024-06-20 13:34:35.656 EDT [1633957][client backend] [pg_regress/skip_scan][0/396:0] LOCATION:  _bt_advance_array_keys, nbtutils.c:3566
 ...
 2024-06-20 13:34:35.656 EDT [1633957][client backend] [pg_regress/skip_scan][0/397:0] STATEMENT:  EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
	execute tenth;
*/
-- Write out the dirty pages left by the insert above, so that the query
-- below never has to evict one itself (which would show up as 'written=1')
checkpoint;
execute tenth;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF, SUMMARY OFF)
execute tenth;
...
--
-- 2026-08-11 Assertion failure "!prefetchBatch->isGuarded" in the prefetchPos
-- catchup path of heapam_index_prefetch_next_block.  An index-only scan
-- resolves each batch's visibility info in runs that double in size (1, 2, 4,
-- ...), so a scan that stops part way through its first batch leaves an
-- unresolved item at the far end of that batch.  When the scan changes
-- direction the catchup path sweeps visibility away from the unresolved item,
-- so the batch never becomes fully resolved -- and so is still guarded when
-- prefetching resumes from it.
--
-- Needs one row per heap page (that's what the padding plus fillfactor buy
-- us), since prefetching only starts on the
-- INDEX_PREFETCH_BLKSWITCH_THRESHOLD'th heap block switch.  All 8 index
-- tuples fit on one leaf page, so the scan has a single batch, whose 8 items
-- outnumber the 1 + 2 + 4 items resolved before the cursor changes direction.
--
set client_min_messages=error;
drop table if exists catchup_vis_gap_bug;
reset client_min_messages;

create table catchup_vis_gap_bug (a int, pad char(1000))
  with (fillfactor = 10);                             -- one row per heap page
create index on catchup_vis_gap_bug (a);
insert into catchup_vis_gap_bug select i, '' from generate_series(0, 7) s(i);
...
begin;
declare visgap scroll cursor for select a from catchup_vis_gap_bug order by a;
fetch forward 3 from visgap;
fetch backward 0 from visgap;
fetch forward 2 from visgap;
-- Assertion fails at heapam_indexscan.c:1366 without the fix
fetch backward all from visgap;
commit;
