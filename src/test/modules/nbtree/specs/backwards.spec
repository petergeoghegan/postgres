...
setup
{
  ...
  CREATE FUNCTION noparallel_func(a INT) RETURNS INT AS $$
    BEGIN
      RETURN a;
    END;
...
# Ensure we don't do a parallel index scan with a parallel unsafe function
step b_scan { SELECT noparallel_func(col) FROM backwards_scan_tbl
