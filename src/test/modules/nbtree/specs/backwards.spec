...
setup {
  ...
  SELECT injection_points_attach('nbtree-lock-and-validate-new-lastcurrblkno', 'notice');
  SELECT injection_points_attach('nbtree-lock-and-validate-left', 'wait');
...
step i_detach {
  SELECT injection_points_detach('nbtree-lock-and-validate-left');
  SELECT injection_points_wakeup('nbtree-lock-and-validate-left');
}
