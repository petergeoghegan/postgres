...
#
# Backwards scans cannot unreservedly trust their saved left link: by the
# time the scan follows it, concurrent page splits and/or page deletions may
# have left it pointing to a page that is no longer the correct page for the
# scan to read next.  The scan checks for this by verifying that the
# pointed-to page's right link still points back to the page that the scan
# just read, and recovers when it doesn't (see nbtree/README for details).
#
# Each permutation makes the scan wait "between pages" at the
# nbtree-walk-left injection point while the concurrent session splits
# and/or deletes pages, then wakes it, steering the scan into one of the
