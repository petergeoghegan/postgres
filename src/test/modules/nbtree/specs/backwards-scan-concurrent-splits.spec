...
# recover from.
#
# The nbtree-walk-left injection point is used to make the scan wait
...
#
# nbtree-walk-left-step-right fires each time the scan's search steps right
# to recover from a concurrent page split (never more than 4 times per
# search attempt).
#
# nbtree-walk-left-restart fires when the search gives up on the
# scan's saved left link/right sibling pages, and starts over using the
# right sibling page's current left link.
#
# nbtree-walk-left-deleted fires when the search finds that the
