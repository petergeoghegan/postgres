...
SELECT * FROM index_scan_tids('hot_chain_tab', 'hot_chain_idx', 'any');

-- MVCC scan, for contrast: only the newest member of each chain
SELECT * FROM index_scan_tids('hot_chain_tab', 'hot_chain_idx', 'mvcc');

-- backward: index entries in reverse key order, chains still in ASC chain order
SELECT * FROM index_scan_tids('hot_chain_tab', 'hot_chain_idx', 'any', 'backward');
