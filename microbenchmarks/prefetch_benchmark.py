...
def server_has_prefetch_guc(conn):
    ...
    enable_indexscan_prefetch GUC.  BUILD_HAS_PREFETCH is the *static* declaration
...
def print_missing_prefetch_guc_warning(testbranch_name):
    ...
    enable_indexscan_prefetch GUC.  Used both inline (at detection) and again at
    the very end of the run."""
    line = "!" * 72
    print(f"\n{line}")
    print(f"!! WARNING: testbranch '{testbranch_name}' has NO "
          f"enable_indexscan_prefetch GUC,")
...
def verify_test_has_prefetch(conn, test_has_prefetch, testbranch_name):
    ...
    If the build is expected to have enable_indexscan_prefetch but the server
    ...
    instead of erroring out later on SET enable_indexscan_prefetch.  The warning
...
def start_server(pg_bin_dir, pg_name, pg_data_dir, conn_details, args, on_patch):
    ...
    pg_options = [
        ...
        # Keep this value in sync with ~/dotfiles/set-pg-hugepages.sh.
