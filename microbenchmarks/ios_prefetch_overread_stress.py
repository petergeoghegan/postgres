...
test correctly reporting the pathology is fixed.
 ...
 9. Prefetch is eligible: plain MVCC-snapshot IoS with
    enable_indexscan_prefetch=on.  [forced plan + GUCs]
...
# benchmark_common; it only borrows its patterns so it can stand alone).
# ---------------------------------------------------------------------------
...
def clear_os_cache():
    """Drop the OS page cache (sudo clear_cache.sh -- passwordless on this host)."""
...
def set_session(cur, prefetch, force_gucs, eic=None, combine=None):
    ...
    if prefetch is not None:  # None => build lacks the GUC (e.g. master)
        cur.execute(f"SET enable_indexscan_prefetch = {prefetch}")
    if eic is not None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            has_guc = q1(cur, "SELECT 1 FROM pg_settings "
                              "WHERE name='enable_indexscan_prefetch'")
        facts = sc.build(conn)
        with conn.cursor() as cur:
        ...
        with conn.cursor() as cur:
            ...
            # enable_indexscan_prefetch is an execution-time switch, so the same
            ...
            for mode in modes:
                for vlabel, pref in variants:
                    ...
                    if pref is not None:
                        cur.execute(f"SET enable_indexscan_prefetch = {pref}")
...
def main():
    ...
    if not has_guc and args.build != "master":
        cluster.cleanup()
        sys.exit(f"build at {args.bindir} has no enable_indexscan_prefetch GUC "
