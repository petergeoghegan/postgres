...
# --- Script variables ---
FLAMEGRAPH_DIR = os.path.expanduser("~/code/FlameGraph")
...
def parse_arguments():
    ...
    prefetch_group.add_argument(
        ...
        action="store_true",
        dest="prefetch_only",
        help="Set debug_disable_indexscan_prefetch=off (prefetching on) on patch (use with --queries)"
    )
    prefetch_group.add_argument(
        "--prefetch-disabled",
        action="store_true",
        dest="prefetch_disabled",
        help="Set debug_disable_indexscan_prefetch=on (prefetching off) on patch (use with --queries)"
    )
    parser.add_argument(
        "--baseline",
        type=str,
...
def profile_postgres(pg_bin_dir, pg_name, conn_details, output_file, sql_query, query_repetitions, run_perf, max_aid_val=None, perf_event=None, highfreq=False, index_only_scan=False, run_perfstat=False, discard_runs=0, benchmark_cpu=14, perf_cpu=15, disable_prefetch=False, is_master=False, prefetch_setting=None, no_pin=False, use_bitmap=False, bitmap_range=2000):
    ...
    try:
        ...
        with conn.cursor() as cursor:
            # if pg_name == "patch":
            #     cursor.execute("set debug_disable_indexscan_prefetch=on;")
            if use_bitmap:
            ...
            if not is_master and prefetch_setting is not None:
                cursor.execute("set debug_disable_indexscan_prefetch = "
